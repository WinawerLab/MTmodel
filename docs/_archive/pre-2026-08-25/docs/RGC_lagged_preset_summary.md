...
`docs/_archive/RGC_V1_unification_plan.md` §3.5). It replaces the offset+quadrature preset now
...
This document describes the **front-end only**. How it is validated, how it
drives MT, and what the lesion tests showed are in
...
> **What the midget/parasol labels buy, and only since 2026-08-14.** Fitting this
> 160-feature basis to SH's V1 does *not* by itself make the cell-type labels
> mean anything — that objective has no M/P information in it, and the fit came
> out midget-dominated, which is Maunsell et al. (1990) backwards. The labels
> acquire meaning at MT, via the two-stream mixture `pars.rgc.mtMix`
> (parasol-masked fast stream + mixed slow stream, `alpha = 0.10`). **Any
> cell-type-specific claim from this preset requires that mixture to be set.**
> See `MODEL_AND_LESIONS.md` §2.5.
...
4. **A later measurement disagrees.** `explore/fitMagnoMtPopulation.m`
   (2026-08-14) put the same mixed population at median *r* = 0.93–0.95 on a
   different stimulus with differently-cached weights. The two are not directly
   comparable, but **~0.985 should not be quoted as a stable property of the
   preset** until the discrepancy is resolved.
